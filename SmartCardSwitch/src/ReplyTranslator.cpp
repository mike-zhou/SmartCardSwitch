...
ReplyTranslator::ReplyTranslator(const std::string& reply)
{
	...
	_stepperForwardClockwisePtr = nullptr;
...
void ReplyTranslator::parseReply(Poco::JSON::Object::Ptr objectPtr, const std::string& command)
{
	...
	{
		_type = ReplyType::StepperForwardClockwise;

		std::shared_ptr<ReplyStepperForwardClockwise> ptr (new ReplyStepperForwardClockwise);
		//common attributes
		ptr->originalString = _reply;
		ptr->commandKey = commandKey;
		ptr->commandId = commandId;
		...
		if(errorInfo.empty()) {
			ptr->index = ds["index"];
			ptr->forwardClockwise = ds["forwardClockwise"];
		}

		_stepperForwardClockwisePtr = ptr;
...
std::shared_ptr<ReplyTranslator::ReplyStepperForwardClockwise> ReplyTranslator::ToStepperForwardClockwise()
{
	return _stepperForwardClockwisePtr;
}
