function onElementClicked() 
{
    ...
    else if (group === "adjustStepperU") 
    {
        var action = paraArray[1];
        
        if(action === "decreaseOffset") 
        {
            var offset = parseInt(document.getElementById("adjustStepperU_offsest").innerText);
            offset = offset - 1;

            var xhr = new XMLHttpRequest();
            xhr.responseType = "text/plain";
            xhr.open('POST', '/adjustStepperU');
            ...
            xhr.onreadystatechange = function() {
                ...
                if (xhr.readyState === DONE) {
                    ...
                    if (xhr.status === OK) {
                        document.getElementById("adjustStepperU_offsest").innerText = offset;
                        console.log("current stepper U offset: " + offset);
                    } else {
                        alert('Error: ' + xhr.status + ":" + xhr.statusText + ":" + xhr.response); // An error occurred during the request.
                    }
                }
        ...
        else if(action === "finish") 
        {
            ...
            xhr.onreadystatechange = function() {
                ...
                if (xhr.readyState === DONE) {
                    ...
                    if (xhr.status === OK) {
                        console.log("finished stepper U adjusting");
                    } else {
                        alert('Error: ' + xhr.status + ":" + xhr.statusText + ":" + xhr.response); // An error occurred during the request.
                    }
                }
